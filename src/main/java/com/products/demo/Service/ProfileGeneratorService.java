...
public class ProfileGeneratorService {
    ...
    private static final Pattern LOG_PATTERN = Pattern.compile(
            "\\[(?:requestId=(\\S+?), )?username=(\\S+?), operation=(\\S+?), entity=(\\S+?)(?:, resourceId=(\\S+?))?(?:, priceThreshold=(\\S+?))?\\] - Performing (\\S+?) operation: (\\S+?) on (\\S+?), user: (\\S+)"
    );
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(LOG_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (matcher.find()) {
                    ...
                    profiles.computeIfAbsent(username, k -> new UserProfile(username)).addOperation(
                            operation, entity, resourceId, priceThreshold, timestamp
                    );
                }
            }
    ...
    private void saveProfile(UserProfile profile) {
        try {
            ...
            java.io.File outDir = new java.io.File(OUTPUT_DIR);
            if (!outDir.exists()) {
    ...
    public static class UserProfile {
        ...
        public void addOperation(String operation, String entity, String resourceId, String priceThreshold, String timestamp) {
            String opKey = operation.equals("READ") ? "readOperations" :
                    operation.equals("WRITE") ? "writeOperations" : "expensiveProductSearches";
        ...
        public void setOperationStats(Map<String, OperationStats> operationStats) { this.operationStats = operationStats; }
    ...
    public static class OperationStats {
        ...
        public void addDetail(OperationDetail detail) { this.details.add(detail); }

        public int getCount() { return count; }
        public void setCount(int count) { this.count = count; }
        public List<OperationDetail> getDetails() { return details; }
        public void setDetails(List<OperationDetail> details) { this.details = details; }
    }
