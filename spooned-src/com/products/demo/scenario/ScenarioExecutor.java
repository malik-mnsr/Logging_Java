...
public class ScenarioExecutor {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            for (int s = 1; s <= 20; s++) {
                ...
                switch (scenario) {
                    ...
                    case 3 ->
                        getProductById(token, random.nextInt(15) + 1);
                    case 4 ->
                        deleteProduct(token, random.nextInt(15) + 1);
                    case 5 ->
                        getExpensiveProducts(token);
                    case 6 ->
                        updateProduct(token, random.nextInt(15) + 1);
                    case 7 ->
                        searchByPrice(token, random.nextDouble(50));
                }
    ...
    private static HttpResponse<String> sendGet(String path, String token) throws Exception {
        HttpRequest.Builder req = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080" + path)).GET();
        if (token != null)
    ...
    private static HttpResponse<String> sendPost(String path, String body, String token) throws Exception {
        HttpRequest.Builder req = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080" + path)).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body));
        if (token != null)
            req.setHeader("Authorization", "Bearer " + token);
    ...
    private static HttpResponse<String> sendPut(String path, String body, String token) throws Exception {
        HttpRequest.Builder req = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080" + path)).header("Content-Type", "application/json").PUT(HttpRequest.BodyPublishers.ofString(body));
        if (token != null)
            req.setHeader("Authorization", "Bearer " + token);
    ...
    private static HttpResponse<String> sendDelete(String path, String token) throws Exception {
        HttpRequest.Builder req = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080" + path)).DELETE();
        if (token != null)
            req.setHeader("Authorization", "Bearer " + token);
